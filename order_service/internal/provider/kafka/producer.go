...
func createTopicWithRetry(conn *kafka.Conn, topic string, partitions int, replicationFactor int, retries int) error {
	for i := 0; i < retries; i++ {
		err := createTopic(conn, topic, partitions, replicationFactor)
		if err == nil {
			return nil // Successfully created the topic
		}
		log.Warn().Msgf("Retrying topic creation (%d/%d) due to error: %v", i+1, retries, err)
		time.Sleep(time.Duration(2^i) * time.Second) // Exponential backoff
	}
	return fmt.Errorf("failed to create topic '%s' after %d retries", topic, retries)
}

func NewKafkaProducer(brokers []string, topic string, partitions int, replicationFactor int) (*KafkaProducer, error) {
    conn, err := kafka.Dial("tcp", brokers[0])
	log.Debug().Msgf("topic received is %s", topic)
    if err != nil {
